...
export const getStores = async (req, res, next) => {
    try {
        ...
        let assignedToolIds = null;

        if (req.user && req.user.role === 'Vendor') {
            const scope = await getVendorAssignedScope(req.user);
            assignedStoreIds = scope.assignedStoreObjectIds;
            assignedToolIds = scope.assignedToolObjectIds;

            if (req.query.projectId && !scope.assignedProjectIds.includes(req.query.projectId.toString())) {
        ...
        const stores = await storeService.getStores(req.query.projectId, assignedStoreIds, assignedToolIds);
...
export const getStoreById = async (req, res, next) => {
    try {
        ...
        if (req.user && req.user.role === 'Vendor') {
            const scope = await getVendorAssignedScope(req.user);
            if (!scope.assignedStoreIds.includes(req.params.id.toString())) {
        ...
        const store = await storeService.getStoreById(req.params.id, assignedToolIds);
        if (!store) {
